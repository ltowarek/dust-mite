...
# Top-left: C++ build → flash → monitor
byobu send-keys -t 'dust-mite:0.{top-left}' \
  'docker compose exec cpp /workspaces/dust-mite/scripts/run_cpp.sh; exec bash' Enter

# Top-right: Python streamer
byobu send-keys -t 'dust-mite:0.{top-right}' \
  'docker compose exec python /workspaces/dust-mite/scripts/run_python.sh; exec bash' Enter

# Bottom-left: JS dev server
byobu send-keys -t 'dust-mite:0.{bottom-left}' \
  'docker compose exec js /workspaces/dust-mite/scripts/run_js.sh; exec bash' Enter
